...
// UI Constants
const BTN = { x: (GW - 130) / 2, y: 400, w: 130, h: 56 }; // 게임 시작 버튼 (너비 반으로 줄임 260 -> 130, 중앙 정렬)
...
const ATTENDANCE_BTN = { x: GW - 80, y: GH - 160, w: 60, h: 60 }; // 출석체크 버튼 (옵션 버튼 위로 배치)
const COLLECTION_BTN = { x: 20, y: GH - 80, w: 60, h: 60 }; // 도감 버튼 (좌하단)
const OPTIONS_BTN = { x: GW - 80, y: GH - 80, w: 60, h: 60 }; // 옵션 버튼 (우하단)
const OPT_RESET_BTN = { x: 50, y: GH - 110, w: GW - 100, h: 45 }; // 옵션 초기화 버튼
...
const BOMB_VX = 2.75, BOMB_VY = -12.0; // 폭탄 던지는 속도 (비거리 5.5 -> 2.75로 조정)
...
window.GameState = {
    ...
    // 옵션 항목 정의 (GameState 내부에 직접 포함)
    OPTION_ITEMS: [
        { label: 'BGM', key: 'bgmEnabled', type: 'toggle' },
        { label: 'BGM 볼륨', key: 'bgmVolume', type: 'slider', min: 0, max: 100, isPercent: true },
        { label: '효과음', key: 'sfxEnabled', type: 'toggle' },
        { label: '효과음 볼륨', key: 'sfxVolume', type: 'slider', min: 0, max: 100, isPercent: true },
        { label: '클리어 거리', key: 'clearDistance', type: 'slider', min: 100, max: 1000, suffix: 'm' },
        { label: '그래픽 품질', key: 'graphicsQuality', type: 'select', options: ['low', 'medium', 'high'] },
        { label: '전체화면', key: 'fullscreen', type: 'toggle' }
    ],
    collectionData: {},
    isPaused: false,
