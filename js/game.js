...
let reloadCooldown = 0; // 재장전 쿨타임 (프레임)
...
let prevBgmEnabled = options.bgmEnabled;
...
// 옵션 입력/키보드 처리용 메타 (UI 렌더 순서와 동일)
function getOptionItems() {
  return [
    { key: 'bgmEnabled', type: 'toggle' },
    { key: 'bgmVolume', type: 'slider', min: 0, max: 100, value: function () { return Math.round(options.bgmVolume * 100); } },
    { key: 'sfxEnabled', type: 'toggle' },
    { key: 'sfxVolume', type: 'slider', min: 0, max: 100, value: function () { return Math.round(options.sfxVolume * 100); } },
    { key: 'clearDistance', type: 'slider', min: 100, max: 1000, value: function () { return options.clearDistance; } },
    { key: 'graphicsQuality', type: 'select', options: ['low', 'medium', 'high'] },
...
function applyOptions() {
  ...
  // BGM 토글(on/off) 변경시에만 재생 상태를 바꾼다.
  // 다른 옵션 조정에서는 BGM 재시작/전환을 하지 않는다.
  const bgmToggled = options.bgmEnabled !== prevBgmEnabled;
  if (bgmToggled) {
    if (!options.bgmEnabled) {
      if (bgmTitle) bgmTitle.pause();
      if (bgmStage) bgmStage.pause();
    } else if (state === 'playing' || state === 'stage1clear') {
      if (bgmTitle) bgmTitle.pause();
      if (bgmStage) bgmStage.play().catch(function () { });
    } else {
      if (bgmStage) bgmStage.pause();
      if (bgmTitle) bgmTitle.play().catch(function () { });
    }
  }
  prevBgmEnabled = options.bgmEnabled;
}
...
// 초기 로드
loadCollectionData();
// 스폰 y 5포인트 (하→상, f.y=음식 상단): [0]발과 같은 높이, [1～3]탄환으로 맞출 수 있는 높이, [4]점프·탄환으로 맞출 수 없고 닿으면 체력 감소
...
function updateGifPositions() {
  ...
  // 단일 애니메이션만 표시되도록 강제
  // 우선순위: bomb > shoot > slide > run
  let activeAnim = 'none';
  if (state === 'playing' && hp > 0 && deathFallFrames === 0 && pauseFramesLeft === 0) {
    if (bombActive) activeAnim = 'bomb';
    else if (shootActive) activeAnim = 'shoot';
    else if (slideActive) activeAnim = 'slide';
    else activeAnim = 'run';
  }
  const shouldShowGirlGif = (activeAnim === 'run');
  const shouldShowSlideGif = (activeAnim === 'slide');
  const shouldShowShootGif = (activeAnim === 'shoot');
  const shouldShowBombGif = (activeAnim === 'bomb');
...
function drawBullet(b) {
  ...
  ctx.font = '24px "Noto Color Emoji", "Segoe UI Emoji", sans-serif';
  ctx.textAlign = 'center';
  ctx.textBaseline = 'middle';
  ctx.fillText('🚀', 0, 0);
  ctx.restore();

  // drawBomb/drawFood 등 다른 렌더러에 영향 없도록 기본 정렬 복원
  ctx.textAlign = 'left';
...
function addBomb() {
  ...
  // 새 애니메이션 시작 시 기존 캐릭터 애니메이션 즉시 종료
  if (shootActive) {
    shootActive = false;
    shootFrameCount = 0;
  }
...
function handleTap(e) {
  ...
  // 타이틀 화면에서만 하단 아이콘 버튼 허용
  const allowTitleButtons = (state === 'start');

  if (allowTitleButtons && isInBtn(pos, COLLECTION_BTN)) {
    if (state === 'playing') {
      isPaused = true;
    }
    state = 'collection';
  ...
  if (allowTitleButtons && isInBtn(pos, OPTIONS_BTN)) {
    if (state === 'playing') {
      isPaused = true;
    }
    state = 'options';
  ...
  if (allowTitleButtons && isInBtn(pos, ATTENDANCE_BTN)) {
...
document.addEventListener('keydown', function (e) {
  ...
  if (state === 'options') {
    ...
    if (e.code === 'ArrowDown') {
      e.preventDefault();
      selectedOptionIndex = Math.min(optionItems.length - 1, selectedOptionIndex + 1);
      return;
    }
    if (e.code === 'ArrowLeft') {
      e.preventDefault();
      const item = optionItems[selectedOptionIndex];
      if (item.type === 'slider') {
        const currentValue = item.value();
        const step = (item.key === 'clearDistance') ? 50 : 5;
        const newValue = Math.max(item.min, currentValue - step);
        if (item.key === 'bgmVolume') {
          ...
          options.clearDistance = Math.round(newValue / 50) * 50;
        }
        saveOptions();
      } else if (item.type === 'select') {
        const currentIndex = item.options.indexOf(options[item.key]);
    ...
    if (e.code === 'ArrowRight') {
      ...
      if (item.type === 'slider') {
        const currentValue = item.value();
        const step = (item.key === 'clearDistance') ? 50 : 5;
        const newValue = Math.min(item.max, currentValue + step);
        if (item.key === 'bgmVolume') {
          options.bgmVolume = newValue / 100;
        } else if (item.key === 'sfxVolume') {
          options.sfxVolume = newValue / 100;
